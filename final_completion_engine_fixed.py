    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if admin.role not in roles:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Role denied. Required: {roles}"
                )
            return func(*args, **kwargs)
        return wrapper
    return decorator
...
'''
        ...
        if 'import' in lines[0] or 'from' in lines[0]:
            ...
            lines.insert(last_import, rbac_setup)
        else:
            lines.insert(0, rbac_setup)
...
'''
        ...
        print(f"  ✓ Added RBAC to Python file {main_file}")
    
    def add_rbac_to_go(self, main_file: Path, service_name: str):
        """Add RBAC to Go file"""
        content = main_file.read_text(encoding='utf-8', errors='ignore')
...
const (
    ...
    USER_SUSPEND        Permission = "user:suspend"
    USER_VERIFY         Permission = "user:verify"
    WITHDRAWAL_APPROVE  Permission = "withdrawal:approve"
    WITHDRAWAL_REJECT   Permission = "withdrawal:reject"
    DEPOSIT_MONITOR     Permission = "deposit:monitor"
    TRANSACTION_REVIEW  Permission = "transaction:review"
    FEE_MANAGE          Permission = "fee:manage"
    TRADING_HALT        Permission = "trading:halt"
    TRADING_RESUME      Permission = "trading:resume"
    PAIR_MANAGE         Permission = "pair:manage"
    LIQUIDITY_MANAGE    Permission = "liquidity:manage"
    RISK_CONFIGURE      Permission = "risk:configure"
    POSITION_MONITOR    Permission = "position:monitor"
    LIQUIDATION_MANAGE  Permission = "liquidation:manage"
    SYSTEM_CONFIG       Permission = "system:config"
    FEATURE_FLAG        Permission = "feature:flag"
    MAINTENANCE_MODE    Permission = "maintenance:mode"
    KYC_APPROVE         Permission = "kyc:approve"
    KYC_REJECT          Permission = "kyc:reject"
    AML_MONITOR         Permission = "aml:monitor"
    COMPLIANCE_REPORT   Permission = "compliance:report"
    ANNOUNCEMENT_CREATE Permission = "announcement:create"
    ANNOUNCEMENT_UPDATE Permission = "announcement:update"
    ANNOUNCEMENT_DELETE Permission = "announcement:delete"
    ANALYTICS_VIEW      Permission = "analytics:view"
    REPORT_GENERATE     Permission = "report:generate"
    AUDIT_LOG_VIEW      Permission = "audit:view"
)

// Role permissions mapping
var ROLE_PERMISSIONS = map[UserRole][]Permission{{
    SUPER_ADMIN: {{USER_VIEW, USER_CREATE, USER_UPDATE, USER_DELETE, USER_SUSPEND, USER_VERIFY,
        WITHDRAWAL_APPROVE, WITHDRAWAL_REJECT, DEPOSIT_MONITOR, TRANSACTION_REVIEW, FEE_MANAGE,
        TRADING_HALT, TRADING_RESUME, PAIR_MANAGE, RISK_CONFIGURE, POSITION_MONITOR,
        SYSTEM_CONFIG, KYC_APPROVE, KYC_REJECT, ANNOUNCEMENT_CREATE, ANNOUNCEMENT_UPDATE,
        ANALYTICS_VIEW, REPORT_GENERATE, AUDIT_LOG_VIEW}},
    ADMIN: {{USER_VIEW, USER_CREATE, USER_UPDATE, USER_SUSPEND, USER_VERIFY,
        WITHDRAWAL_APPROVE, WITHDRAWAL_REJECT, DEPOSIT_MONITOR, TRANSACTION_REVIEW, FEE_MANAGE,
        TRADING_HALT, TRADING_RESUME, PAIR_MANAGE, RISK_CONFIGURE, POSITION_MONITOR,
        SYSTEM_CONFIG, KYC_APPROVE, KYC_REJECT, ANNOUNCEMENT_CREATE, ANNOUNCEMENT_UPDATE,
        ANALYTICS_VIEW, REPORT_GENERATE, AUDIT_LOG_VIEW}},
    MODERATOR: {{USER_VIEW, USER_SUSPEND, TRANSACTION_REVIEW, KYC_APPROVE,
        ANNOUNCEMENT_CREATE, ANALYTICS_VIEW}},
    SUPPORT: {{USER_VIEW, TRANSACTION_REVIEW, ANALYTICS_VIEW}},
    COMPLIANCE: {{USER_VIEW, USER_VERIFY, WITHDRAWAL_APPROVE, WITHDRAWAL_REJECT,
        TRANSACTION_REVIEW, KYC_APPROVE, KYC_REJECT, AML_MONITOR, COMPLIANCE_REPORT,
        AUDIT_LOG_VIEW}},
    RISK_MANAGER: {{POSITION_MONITOR, RISK_CONFIGURE, LIQUIDATION_MANAGE, TRADING_HALT,
        ANALYTICS_VIEW, REPORT_GENERATE}},
    TRADER: {{}},
    USER: {{}},
}}
...
func getCurrentAdmin() *AdminUser {{
    return &AdminUser{{
        UserID:    "admin_001",
        Username:  "admin",
        Email:     "admin@tigerex.com",
...
func hasPermission(user *AdminUser, permission Permission) bool {{
...
func requirePermission(permission Permission, handler http.HandlerFunc) http.HandlerFunc {{
    return func(w http.ResponseWriter, r *http.Request) {{
        admin := getCurrentAdmin()
        if !hasPermission(admin, permission) {{
            http.Error(w, fmt.Sprintf("Permission denied. Required: %s", permission), http.StatusForbidden)
            return
        }}
        handler(w, r)
    }}
...
func requireRole(roles []UserRole, handler http.HandlerFunc) http.HandlerFunc {{
    return func(w http.ResponseWriter, r *http.Request) {{
        admin := getCurrentAdmin()
        ...
        http.Error(w, fmt.Sprintf("Role denied. Required: %v", roles), http.StatusForbidden)
    }}
}}
...
'''
        ...
        lines = content.split('\n')
        if 'import' in lines[0] or 'package' in lines[0]:
            ...
            lines.insert(insert_pos + 1, rbac_code)
        else:
            lines.insert(0, 'import "time"')
            lines.insert(1, rbac_code)
        
        content = '\n'.join(lines)
        main_file.write_text(content, encoding='utf-8')
        self.results["rbac_completed"] += 1
        print(f"  ✓ Added RBAC to Go file {main_file}")
