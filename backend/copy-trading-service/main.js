...
app.get('/api/v1/copy-trading/masters', authenticateUser, async (req, res) => {
    try {
        const { page = 1, limit = 20, sort_by = 'total_return', order = 'desc' } = req.query;
...
app.get('/api/v1/copy-trading/masters/:masterId', authenticateUser, async (req, res) => {
    try {
...
app.post('/api/v1/copy-trading/subscribe', authenticateUser, async (req, res) => {
    try {
        const {
            master_trader_id,
            allocated_amount,
        ...
        if (masterResult.rows.length === 0) {
            return res.status(404).json({ error: 'Master trader not found or inactive' });
        }
        ...
        if (balanceResult.rows.length === 0 || balanceResult.rows[0].balance < allocated_amount) {
        ...
        if (existingSubscription.rows.length > 0) {
            return res.status(400).json({ error: 'Already subscribed to this master trader' });
        }
        ...
        `, [
            subscriptionId, req.user.id, master_trader_id, allocated_amount,
            copy_percentage || 100, max_trade_amount, stop_loss_percentage,
            take_profit_percentage, JSON.stringify(risk_management_settings || {}),
...
app.get('/api/v1/copy-trading/subscriptions', authenticateUser, async (req, res) => {
    try {
        const query = `
            SELECT 
                cts.*,
        ...
        const subscriptions = result.rows.map(sub => ({
            ...sub,
            risk_management_settings: JSON.parse(sub.risk_management_settings || '{}')
        }));
...
app.put('/api/v1/copy-trading/subscriptions/:subscriptionId', authenticateUser, async (req, res) => {
    try {
        ...
        if (allocated_amount && allocated_amount !== current.allocated_amount) {
            ...
            if (difference > 0) {
                ...
                if (balanceResult.rows.length === 0 || balanceResult.rows[0].balance < difference) {
                    await db.query('ROLLBACK');
                    return res.status(400).json({ error: 'Insufficient balance for increased allocation' });
                }
            }
...
app.delete('/api/v1/copy-trading/subscriptions/:subscriptionId', authenticateUser, async (req, res) => {
    try {
        const { subscriptionId } = req.params;
        ...
        res.status(500).json({ error: 'Failed to unsubscribe from copy trading' });
    }
});

// Apply to become a master trader
app.post('/api/v1/copy-trading/apply-master', authenticateUser, async (req, res) => {
    try {
        const {
            strategy_name,
        ...
        if (existingResult.rows.length > 0) {
            return res.status(400).json({ error: 'Already applied or approved as master trader' });
        }

        // Check user's trading history and KYC status
        if (req.user.kyc_level < 2) {
            return res.status(400).json({ error: 'KYC level 2 required to become a master trader' });
        }
        ...
        `, [
            applicationId, req.user.id, strategy_name, strategy_description,
            JSON.stringify(trading_style), risk_level, minimum_followers || 0,
            performance_fee_percentage || 0, JSON.stringify(track_record || {})
        ]);
        ...
        res.status(500).json({ error: 'Failed to submit master trader application' });
    }
});

// Get copy trading analytics
app.get('/api/v1/copy-trading/analytics', authenticateUser, async (req, res) => {
    try {
        ...
        const totalAllocated = performanceResult.rows.reduce((sum, sub) => sum + parseFloat(sub.allocated_amount), 0);
        const totalPnL = performanceResult.rows.reduce((sum, sub) => sum + parseFloat(sub.profit_loss_amount || 0), 0);
        const avgReturn = performanceResult.rows.length > 0 
            ? performanceResult.rows.reduce((sum, sub) => sum + parseFloat(sub.profit_loss_percentage || 0), 0) / performanceResult.rows.length
            : 0;
...
class TradeSignalProcessor {
    ...
    async executeCopiedTrade(signal, followerSubscription) {
        try {
            ...
            const allocationRatio = parseFloat(followerSubscription.allocated_amount) / 10000; // Assuming master has $10k base
            const copyPercentage = parseFloat(followerSubscription.copy_percentage) / 100;
            
            let tradeSize = baseTradeSize * allocationRatio * copyPercentage;

            // Apply max trade amount limit
            if (followerSubscription.max_trade_amount && tradeSize > followerSubscription.max_trade_amount) {
            ...
            `, [
                copiedTradeId, followerSubscription.follower_id, signal.master_trader_id,
                signal.id, followerSubscription.id, signal.trading_pair_id,
                signal.order_type, signal.side, tradeSize, signal.price
            ]);
    ...
    async executeTradeOrder(order) {
        try {
            ...
            const response = await axios.post(`${process.env.TRADING_ENGINE_URL}/api/v1/orders`, {
