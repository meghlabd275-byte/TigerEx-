// React Trading Dashboard Web App
...
const platformLinks = {
  ...
  assets: [
    ...
    { name: 'Futures Wallet', url: 'https://www.bitget.com/asset', icon: '📈', desc: 'Futures' },
...
function App() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: t.bg, color: t.text, fontFamily: 'system-ui' }}>
      {/* Sidebar */}
      <nav style={{ width: 260, background: t.surface, borderRight: '1px solid rgba(255,255,255,0.06)', padding: 20, position: 'fixed', height: '100vh', overflowY: 'auto' }}>
        <h1 style={{ fontSize: '1.8rem', color: '#F6821F', marginBottom: 24 }}>🐯 TigerEx</h1>
        {navItems.map(item => (
          <button key={item.id} onClick={() => setSection(item.id)}
            style={{ display: 'flex', alignItems: 'center', width: '100%', padding: '12px 16px', border: 'none', borderRadius: 8, cursor: 'pointer', background: section === item.id ? 'rgba(246,130,31,0.15)' : 'transparent', color: section === item.id ? '#F6821F' : t.textSec, marginBottom: 4, fontSize: '1rem', textAlign: 'left', transition: 'all 0.2s' }}>
            <span style={{ marginRight: 12 }}>{item.icon}</span>{item.label}
          </button>
        ))}
        <button onClick={() => setDarkMode(!darkMode)} style={{ position: 'absolute', bottom: 20, left: 20, padding: '12px 16px', border: 'none', borderRadius: 8, cursor: 'pointer', background: t.card, color: '#F6821F', fontWeight: 600 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {links.map((link, i) => (
            <a key={i} href={link.url} target="_blank" rel="noopener noreferrer"
              style={{ display: 'block', padding: 20, background: t.card, borderRadius: 12, textDecoration: 'none', color: t.text, transition: 'all 0.2s', border: '1px solid transparent' }}>
