...
const SpotTradingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const mockOrderBook = {
      bids: [
        { price: 45230.00, quantity: 0.5234, total: 23678.12 },
        { price: 45225.00, quantity: 1.2345, total: 55823.45 },
        { price: 45220.00, quantity: 0.8765, total: 39645.23 },
        { price: 45215.00, quantity: 2.1234, total: 96012.34 },
        { price: 45210.00, quantity: 0.6789, total: 30698.45 }
      ],
      asks: [
        { price: 45235.00, quantity: 0.4321, total: 19543.21 },
        { price: 45240.00, quantity: 0.9876, total: 44678.90 },
        { price: 45245.00, quantity: 1.5432, total: 69823.45 },
        { price: 45250.00, quantity: 0.7654, total: 34634.56 },
        { price: 45255.00, quantity: 1.2109, total: 54789.12 }
      ]
    };

    const mockTrades: Trade[] = [
      { id: '1', price: 45234.56, quantity: 0.1234, time: '14:32:15', side: 'buy' },
      { id: '2', price: 45232.10, quantity: 0.5678, time: '14:32:10', side: 'sell' },
      { id: '3', price: 45235.00, quantity: 0.2345, time: '14:32:05', side: 'buy' },
      { id: '4', price: 45230.50, quantity: 0.8901, time: '14:32:00', side: 'sell' },
      { id: '5', price: 45236.78, quantity: 0.3456, time: '14:31:55', side: 'buy' }
    ];
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Trade cryptocurrencies with zero fees</p>
        </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <div className={`text-xs ${getChangeColor(pair.change24h)}`}>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className={`flex items-center space-x-1 ${getChangeColor(selectedPair.change24h)}`}>
                    ...
                    <div className="flex items-center space-x-6 text-sm">
                      <div>
                        ...
                        <div className="font-semibold">${selectedPair.high24h.toLocaleString()}</div>
                      </div>
                      <div>
                        <div className="text-gray-500">24h Low</div>
                        <div className="font-semibold">${selectedPair.low24h.toLocaleString()}</div>
                      </div>
                      <div>
                        <div className="text-gray-500">24h Volume</div>
                        <div className="font-semibold">{selectedPair.volume24h.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <CardContent className="p-4 h-96">
                <div className="w-full h-full bg-gray-100 rounded-lg flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-400">Advanced charting tools coming soon</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {orderBook.asks.reverse().map((ask, index) => (
                        <div key={index} className="flex justify-between text-sm px-4 py-1 hover:bg-red-50">
                    ...
                    <div className="space-y-1">
                      {orderBook.bids.map((bid, index) => (
                        <div key={index} className="flex justify-between text-sm px-4 py-1 hover:bg-green-50">
                ...
                <CardContent className="p-0">
                  <div className="space-y-1">
                    {recentTrades.map((trade) => (
                      <div key={trade.id} className="flex justify-between text-sm px-4 py-2 hover:bg-gray-50">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex space-x-2">
                  <Button size="sm" variant="outline" onClick={() => setOrderQuantity('0.25')}>
                    25%
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setOrderQuantity('0.50')}>
                    50%
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setOrderQuantity('0.75')}>
                    75%
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setOrderQuantity('1.00')}>
                    100%
                  </Button>
                </div>
                ...
                >
                  {orderSide === 'buy' ? 'Buy' : 'Sell'} {selectedPair?.baseAsset}
                </Button>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="p-0">
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {userOrders.filter(order => order.status !== 'filled' && order.status !== 'cancelled').map((order) => (
                    <div key={order.id} className="p-3 border-b last:border-b-0">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        <div>Qty: {order.quantity} / Filled: {order.filled}</div>
                        {order.price && <div>Price: ${order.price.toFixed(2)}</div>}
                        <div>Status: 
                          <Badge className={`ml-1 ${getOrderStatusColor(order.status)}`}>
                  ...
                  {userOrders.filter(order => order.status !== 'filled' && order.status !== 'cancelled').length === 0 && (
